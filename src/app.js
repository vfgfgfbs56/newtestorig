...
import { jendApi } from "./jend.js";
import { clearGuardProof, prepareGuard, takeGuardProof } from "./security.js";
...
async function api(path, options = {}) {
  ...
  return jendApi(path, payload);
}


async function prepareGuestProtection() {
  const status = $("#guest-bot-status");
  try {
    await prepareGuard("qr-create", (text) => { status.textContent = text; });
    if (!$("#guest-view").hidden && !qrState) createQr();
  } catch (error) {
    status.textContent = "Внутренняя защита временно недоступна.";
    qrTitle.textContent = "QR временно недоступен";
    setMessage(
      error?.message === "GUARD_WASM_LOAD_FAILED"
        ? "Не удалось загрузить модуль JEND Guard."
        : "Не удалось выполнить внутреннюю проверку устройства.",
      "error"
    );
  }
}

async function prepareRegisterProtection() {
  const status = $("#register-bot-status");
  try {
    await prepareGuard("register", (text) => { status.textContent = text; });
  } catch {
    status.textContent = "Проверка будет повторена при создании аккаунта.";
  }
}
...
async function createQr() {
  let guardProof = takeGuardProof("qr-create");
  if (!guardProof) {
    qrTitle.textContent = "Проверяем устройство…";
    qrCountdown.textContent = "QR появится после внутренней проверки";
    try {
      await prepareGuard("qr-create", (text) => { $("#guest-bot-status").textContent = text; });
      guardProof = takeGuardProof("qr-create");
    } catch (error) {
      qrTitle.textContent = "QR временно недоступен";
      setMessage(error?.message || "Не удалось выполнить JEND Guard.", "error");
      return;
    }
  }
...
$("#register-view").addEventListener("submit", async (event) => {
  ...
  try {
    let guardProof = takeGuardProof("register");
    if (!guardProof) {
      $("#register-bot-status").textContent = "Проверяем устройство…";
      await prepareGuard("register", (text) => { $("#register-bot-status").textContent = text; });
      guardProof = takeGuardProof("register");
    }
    if (!guardProof) throw new Error("GUARD_PROOF_MISSING");
    ...
    setMessage(
      error?.message === "INVALID_PASSWORD"
        ...
        : error?.message?.startsWith("GUARD")
          ? "Не удалось пройти внутреннюю проверку устройства. Попробуйте ещё раз."
