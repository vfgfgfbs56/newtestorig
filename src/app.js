...
import { ensureTurnstile, getTurnstileToken, resetTurnstile } from "./security.js";
...
async function prepareGuestProtection() {
  try {
    await ensureTurnstile("guest", "#guest-turnstile", "qr-create", (token) => {
      $("#guest-bot-status").textContent = token
        ? "Проверка браузера пройдена."
        : "Проверяем браузер…";
      if (token && !$("#guest-view").hidden && !qrState) createQr();
    });
    if (getTurnstileToken("guest") && !$("#guest-view").hidden && !qrState) createQr();
  } catch (error) {
    $("#guest-bot-status").textContent = "Защита от ботов не настроена.";
    qrTitle.textContent = "QR временно недоступен";
    setMessage(error?.message === "TURNSTILE_NOT_CONFIGURED"
      ? "Добавьте TURNSTILE_SITE_KEY и TURNSTILE_SECRET_KEY в Cloudflare."
      : "Не удалось загрузить защиту от ботов.", "error");
  }
}

async function prepareRegisterProtection() {
  try {
    await ensureTurnstile("register", "#register-turnstile", "register", (token) => {
      $("#register-bot-status").textContent = token
        ? "Проверка браузера пройдена."
        : "Проверяем браузер…";
    });
  } catch (error) {
    $("#register-bot-status").textContent = "Защита от ботов не настроена.";
    setMessage(error?.message === "TURNSTILE_NOT_CONFIGURED"
      ? "Добавьте TURNSTILE_SITE_KEY и TURNSTILE_SECRET_KEY в Cloudflare."
      : "Не удалось загрузить защиту от ботов.", "error");
  }
...
async function createQr() {
  const turnstileToken = getTurnstileToken("guest");
  if (!turnstileToken) {
    qrTitle.textContent = "Проверяем браузер…";
    qrCountdown.textContent = "QR появится после защиты от ботов";
...
$("#register-view").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (!turnstileToken) {
      setMessage("Сначала дождитесь проверки браузера.", "error");
      prepareRegisterProtection();
      return;
    }
