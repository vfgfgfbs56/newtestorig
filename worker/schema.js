...
async function initializeSchema(env) {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS jend_replay (
      request_id TEXT PRIMARY KEY,
      expires_at INTEGER NOT NULL
    )`),
    env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_jend_replay_expires_at ON jend_replay(expires_at)`),
    ...
    env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_rate_limits_expires_at ON rate_limits(expires_at)`),
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS bot_challenges (
      id TEXT PRIMARY KEY,
      action TEXT NOT NULL,
      seed TEXT NOT NULL,
      difficulty INTEGER NOT NULL,
      client_binding TEXT NOT NULL,
      created_at INTEGER NOT NULL,
      expires_at INTEGER NOT NULL,
      used_at INTEGER
    )`),
    env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_bot_challenges_expires_at ON bot_challenges(expires_at)`),
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS guard_devices_v2 (
    ...
    env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_guard_devices_v2_last_seen ON guard_devices_v2(last_seen_ms)`),
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS guard_challenges_v2 (
      id TEXT PRIMARY KEY,
      action TEXT NOT NULL,
      nonce TEXT NOT NULL,
    ...
    env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_guard_challenges_v2_expires ON guard_challenges_v2(expires_at_ms)`),
  ]);
