...
export function normalizeNickname(raw) {
  ...
  if (length < 4 || length > 32) throw new Error("INVALID_NICKNAME");
...
export function qrSignPayload(id, challenge, attemptId) {
  return `secure-auth-qr-v5\n${id}\n${challenge}\n${attemptId}`;
}
...
export function requireSameOrigin(request) {
  ...
  if (!origin || origin !== expected) throw new Error("BAD_ORIGIN");
}
