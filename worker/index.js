...
import { getJendPublicJwk, openJendRequest, wrapJendResponse } from "./jend.js";
import { enforceRateLimit, issueGuardChallenge, verifyGuardProof } from "./security.js";
import { coreVersion } from "./core.js";
...
async function register(request, env) {
  try {
    ...
    if (error?.message === "INVALID_NICKNAME") {
      return json({ ok: false, error: "Юзернейм: 4–32 символа; буквы, цифры, _, . и -." }, 400);
    }
    ...
    if (message.includes("UNIQUE")) {
      return json({ ok: false, error: "Этот юзернейм уже занят." }, 409);
    }
...
async function securityConfig(_request, env) {
  try {
    const publicKeyJwk = getJendPublicJwk(env);
    return json({
      ok: true,
      jend: {
        configured: true,
    ...
    if (["MISSING_JEND_KEY", "INVALID_JEND_KEY"].includes(code)) {
      return json({
        ok: false,
        error: "Не настроен JEND_PRIVATE_JWK.",
        jend: { configured: false },
        guard: { configured: true, version: 2, engine: "server-wasm-c-core", algorithm: "ecdsa-browser-v2" },
      }, 503);
    }
    throw error;
  }
...
async function securityChallenge(request, env) {
  const fetchSite = request.headers.get("Sec-Fetch-Site");
  if (fetchSite && !["same-origin", "same-site", "none"].includes(fetchSite)) {
    return json({ ok: false, error: "Запрос защиты отклонён." }, 403);
  }
  const url = new URL(request.url);
  const action = url.searchParams.get("action") || "";
  const guardId = url.searchParams.get("guard") || "";
  const pressure = await enforceRateLimit(env, request, `guard-${action}`, 30, 60);
  const challenge = await issueGuardChallenge(env, request, action, guardId, pressure);
  return json({ ok: true, ...challenge }, 201);
}

const routes = {
  "/api/security/config": {
    GET: { handler: securityConfig, jend: false },
  },
  "/api/security/challenge": {
    GET: {
      handler: securityChallenge,
      jend: false,
      rate: { action: "guard-challenge", limit: 80, windowSeconds: 10 * 60 },
    },
  },
  "/api/auth/register": {
    POST: {
      ...
      jend: true,
      replay: true,
      rate: { action: "register", limit: 5, windowSeconds: 15 * 60 },
      guardAction: "register",
    },
  },
  "/api/auth/me": {
    GET: { handler: me, jend: false },
  },
  "/api/auth/logout": {
    POST: {
      handler: logout,
      jend: true,
      replay: true,
      rate: { action: "logout", limit: 30, windowSeconds: 60 },
    },
  },
  "/api/auth/qr/create": {
    POST: {
      ...
      jend: true,
      replay: true,
      rate: { action: "qr-create", limit: 30, windowSeconds: 10 * 60 },
      guardAction: "qr-create",
    },
  },
  "/api/auth/qr/pair": {
    POST: {
      ...
      jend: true,
      replay: true,
      rate: { action: "qr-pair", limit: 40, windowSeconds: 5 * 60 },
    },
  },
  "/api/auth/qr/password": {
    POST: {
      handler: qrPassword,
      jend: true,
      replay: true,
      rate: { action: "qr-password", limit: 20, windowSeconds: 2 * 60 },
    },
  },
  "/api/auth/qr/phone-status": {
    POST: { handler: qrPhoneStatus, jend: true, replay: false },
  },
  "/api/auth/qr/result": {
    POST: {
      handler: qrResult,
      jend: true,
      replay: true,
      rate: { action: "qr-result", limit: 20, windowSeconds: 2 * 60 },
    },
  },
  "/api/auth/qr/status": {
    POST: { handler: qrStatus, jend: true, replay: false },
  },
};

function apiErrorResponse(error) {
  const code = String(error?.message || "");
  if (code === "MISSING_DB_BINDING") {
    return json({ ok: false, error: "Не настроена база D1 (binding DB)." }, 503);
  }
  if (["MISSING_JEND_KEY", "INVALID_JEND_KEY"].includes(code)) {
    return json({ ok: false, error: "Не настроен секрет JEND_PRIVATE_JWK." }, 503);
  }
  if (code === "JEND_REQUIRED") {
    return json({ ok: false, error: "Для этого API требуется контейнер JEND v1." }, 415);
  }
  if (["BAD_JEND", "BAD_JEND_AUTH", "BAD_JEND_PAYLOAD", "UNSUPPORTED_JEND_VERSION"].includes(code)) {
    return json({ ok: false, error: "Контейнер JEND отклонён." }, 400);
  }
  if (code === "JEND_EXPIRED") {
    return json({ ok: false, error: "Контейнер JEND устарел. Повторите запрос." }, 408);
  }
  if (code === "JEND_REPLAY") {
    return json({ ok: false, error: "Повторная отправка JEND заблокирована." }, 409);
  }
  if (code === "GUARD_REQUIRED") {
    return json({ ok: false, error: "Требуется проверка JEND Guard." }, 403);
  }
  if (["GUARD_BAD_ACTION", "GUARD_BAD_PROOF", "GUARD_RISK"].includes(code)) {
    return json({ ok: false, error: "Внутренняя проверка JEND Core отклонена." }, 403);
  }
  if (code === "GUARD_TOO_FAST") {
    return json({ ok: false, error: "Проверка отправлена слишком быстро. Повторите попытку." }, 425);
  }
  if (code === "GUARD_BLOCKED") {
    return json({ ok: false, error: "Устройство временно ограничено защитой." }, 429);
  }
  if (code === "GUARD_EXPIRED") {
    return json({ ok: false, error: "Проверка устройства устарела. Повторите попытку." }, 408);
  }
  if (code === "GUARD_REPLAY") {
    return json({ ok: false, error: "Повторное использование проверки устройства заблокировано." }, 409);
  }
  ...
  if (code === "BODY_TOO_LARGE") {
    return json({ ok: false, error: "Запрос слишком большой." }, 413);
  }
  console.error("api_failed", error?.name || "Error", code);
  return json({ ok: false, error: "Ошибка сервера." }, 500);
}

function requestWithJsonPayload(request, payload) {
...
async function handleApi(request, env) {
  ...
  const config = route[request.method];
  if (!config) {
    return json(
  ...
  let jendContext = null;
  try {
    await ensureSchema(env);
    let handlerRequest = request;
    let payload = null;

    // Rate-limit before expensive JEND ECDH/decryption and before replay-state writes.
    if (config.rate) {
      await enforceRateLimit(
        ...
        config.rate.action,
        config.rate.limit,
        config.rate.windowSeconds
      );
    }

    if (config.jend) {
      const opened = await openJendRequest(request, env, {
        replayProtected: config.replay !== false,
      });
      payload = opened.payload;
      jendContext = opened.context;
      handlerRequest = requestWithJsonPayload(request, payload);
    }

    if (config.guardAction) {
      await verifyGuardProof(
        env,
        request,
        payload?.guardProof,
        config.guardAction
    ...
    const response = await config.handler(handlerRequest, env);
    return jendContext ? await wrapJendResponse(jendContext, response) : response;
  } catch (error) {
    const response = apiErrorResponse(error);
    if (jendContext) {
      try {
        return await wrapJendResponse(jendContext, response);
      } catch (wrapError) {
        console.error("jend_response_failed", String(wrapError?.message || ""));
      }
    }
    return response;
  }
