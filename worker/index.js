  ...
  try {
    ...
    return json({
      ...
      guard: {
        ...
        wasmUrl: "/guard/jend-guard-v1.wasm",
      },
    });
  } catch (error) {
    ...
    if (["MISSING_JEND_KEY", "INVALID_JEND_KEY"].includes(code)) {
      return json({
        ...
        guard: { configured: true, version: 1, engine: "wasm-cpp", algorithm: "sha256-pow-v1" },
      }, 503);
    }
    throw error;
  }
...
async function securityChallenge(request, env) {
  ...
  const challenge = await issueGuardChallenge(env, request, action, pressure);
...
function apiErrorResponse(error) {
  ...
  if (["GUARD_BAD_ACTION", "GUARD_BAD_PROOF"].includes(code)) {
    return json({ ok: false, error: "Проверка JEND Guard отклонена." }, 403);
  }
