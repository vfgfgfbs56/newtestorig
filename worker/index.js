...
import { enforceRateLimit, verifyTurnstile } from "./security.js";
...
function apiErrorResponse(error) {
  ...
  if (code === "TURNSTILE_NOT_CONFIGURED") {
    return json({ ok: false, error: "Turnstile ещё не настроен в Cloudflare." }, 503);
  }
  if (code === "TURNSTILE_INVALID") {
    return json({ ok: false, error: "Проверка защиты от ботов не пройдена." }, 403);
  }
